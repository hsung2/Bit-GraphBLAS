...
double evalHSBSFBmv()
{
    ...
    cudaMalloc(&fC, (nblockrows * tiledim) * 1 * sizeof(float));
    setDeviceValArr<int, float><<<1, 1>>>(fC, nblockrows * tiledim, 0);
    ...
    {
#if TILEDIM == 4
        bmv4_bin_full_full<<<grid, 1024>>>(tA, fB, fC, bsrRowPtr, bsrColInd, nblockrows);
#elif TILEDIM == 8
        bmv8_bin_full_full<<<grid, 1024>>>(tA, fB, fC, bsrRowPtr, bsrColInd, nblockrows);
#elif TILEDIM == 16
        bmv16_bin_full_full<<<grid, 1024>>>(tA, fB, fC, bsrRowPtr, bsrColInd, nblockrows);
#elif TILEDIM == 32
        bmv32_bin_full_full<<<grid, 1024>>>(tA, fB, fC, bsrRowPtr, bsrColInd, nblockrows);
#endif
    }
...
void verifyResult()
{
    ...
    cudaMemcpy(result_bsrbmv, fC, nrows * sizeof(float), cudaMemcpyDeviceToHost);

    // verify bsrbmv with cuSPARSE baseline
    printf("success: %d\n", checkResult<float>(result_bsrbmv, result_cusparsecsrspmvfloat, nrows));
...
int main(int argc, char *argv[])
{
    ...
    // hsbsf bmv
    CSR2HSBSF();
    packRandVec(); // randvec host2device
